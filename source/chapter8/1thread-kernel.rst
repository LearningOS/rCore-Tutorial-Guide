...
另外，由于用户态进程与内核之间有各自独立的页表，所以二者需要跳板页
来处理用户态切换到内核态的地址空间转换。其中，地址空间最高处的代码页  ``TRAMPOLINE`` 由属于整个进程（已在第五章介绍）。但出现线程后，在进程中的每个线程需要一个独立的数据页来存储异常陷入时的上下文信息。在 ``sys_thread_create`` 中会为新线程生成 ``TaskUserRes``，其中通过 ``trap_cx_bottom_from_tid`` 指定该线程所属的数据页。后续讲解  ``sys_thread_create`` 时会详细介绍这一点。
...
会回收线程占用的部分资源，即用户态用到的资源，比如用户态的栈，用于系统调用和异常处理的 ``TrapContext`` 跳板页等。
...
但在相关方法上面，由于多个线程有各自的用户栈和用于存放 ``TrapContext`` 的跳板页，所以有些不同，下面会进一步分析。
...
- 线程的跳板页：存放 ``TrapContext`` ，确保线程能正确的进行用户态<-->内核态切换；
...
- 第7-11行，调用 ``TaskControlBlock::new`` 方法，创建一个新的线程 ``new_task`` ，在创建过程中，建立与进程
  ``process`` 的所属关系，分配了线程用户态栈、内核态栈、用于异常/中断时存放 ``TrapContext`` 的页面。
- 第13行，把线程挂到调度队列中。
